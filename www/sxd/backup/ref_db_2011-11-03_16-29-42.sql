#SXD20|20008|50140|50212|2011.11.03 16:29:42|ref_db|0|20|1524|
#TA acl_group_roles`67`1014|acl_roles`34`1468|acl_user_groups`2`88|acl_users`7`524|author`8`816|author_organization`4`28|author_publication`43`473|cities`1321`37060|issue`3`392|issue_type`4`136|keywords`0`0|library_link_list`3`52|library_list`3`156|news`2`172|object_keywords`0`0|organization`3`276|publication`8`1288|publication_type`3`132|publisher`2`72|settings`7`700
#PR deleteAuthorById|deleteAuthorFromPublicationById|deleteIssueById|deleteLibraryList|deleteOrgById|deletePubFromList|deletePublicationById|deletePublisherById|deleteReportBasementById|getAllAuthors|getAllAuthorsByOrg|getAllIssues|getAllIssueTypes|getAllOrgs|getAllPublicationsOfAuthor|getAllPublicationsOfAuthorById|getAllPublicationsOfPublisher|getAllPublicationTypes|getAllPublishers|getAuthor|getAuthorById|getAuthorName|getAuthorsCreatedByUser|getBasementForReport|getIssueById|getIssuesCreatedByUser|getLibraryLists|getNameForReport|getNewPublicationsAmount|getNews|getNewsByID|getOrgById|getOrgsBreadcrumbs|getOrgsByParentId|getOrgsCreatedByUser|getPublicationById|getPublicationFromLibrary|getPublicationFromList|getPublicationsCreatedByAuthor|getPublisherById|getPublishersCreatedByUser|getPubsAmountByAuthorByType|getPubsAmountOfAuthorByType|insertAuthor|insertFirstHead|insertIssue|insertLibraryList|insertLinkToLibrary|insertOrg|insertPublication|insertPublisher|insertToReportBasement|new_proc0|new_proc2|searchByAuthor|searchByName|searchByOrg|searchByPublisher|updateAuthorById|updateIssueById|updateOrgById|updatePublicationById|updatePublicationStatus|updatePublisherById|updateReportBasement|updateReportName|updateReportType
...
(2,3,'тест','тест','тест','2011-10-12','',111111,'as@srs.sss','121','12','')	;
...
) ENGINE=MyISAM AUTO_INCREMENT=19 DEFAULT CHARSET=utf8 COMMENT='таблица публикаций (статей, монографий, методических указани'	;
...
(14,3,1,'hjkhjkertertetertdfgвввввввв',0,2,0,0000,0,0,0,'','0'),
(18,2,1,'sdf',0,2,0,0000,0,0,0,'','0')	;
...
(3,4,'report.head','Муромцев Дмитрий Ильич','','',1),
(4,5,'report.head','Колчин Максим Александрович','','',1),
(5,6,'report.head','Катков Юрий Валериевич','','',1),
...
END	;
#	PR`deleteLibraryList`cp1251_general_ci	;
...
END	;
#	PR`deletePubFromList`cp1251_general_ci	;
...
END	;
#	PR`deleteReportBasementById`cp1251_general_ci	;
...
END	;
#	PR`getAllPublicationsOfAuthor`cp1251_general_ci	;
...
END	;
#	PR`getBasementForReport`cp1251_general_ci	;
...
ORDER BY i.`name` ASC;
END	;
#	PR`getLibraryLists`cp1251_general_ci	;
...
END	;
#	PR`getNameForReport`cp1251_general_ci	;
...
END	;
#	PR`getNewPublicationsAmount`cp1251_general_ci	;
...
END	;
#	PR`getPublicationById`cp1251_general_ci	;
...
p.`id` = ID;
END	;
#	PR`getPublicationFromLibrary`cp1251_general_ci	;
...
l.`list_id` = ID);
END	;
#	PR`getPublicationFromList`cp1251_general_ci	;
...
END	;
#	PR`getPubsAmountByAuthorByType`cp1251_general_ci	;
...
END	;
#	PR`getPubsAmountOfAuthorByType`cp1251_general_ci	;
...
END	;
#	PR`insertFirstHead`cp1251_general_ci	;
...
END	;
#	PR`insertLibraryList`cp1251_general_ci	;
...
END	;
#	PR`insertLinkToLibrary`cp1251_general_ci	;
...
END	;
#	PR`insertPublication`cp1251_general_ci	;
...
END	;
#	PR`insertToReportBasement`cp1251_general_ci	;
...
END	;
#	PR`new_proc0`cp1251_general_ci	;
...
END	;
#	PR`new_proc2`cp1251_general_ci	;
CREATE PROCEDURE `new_proc2`(
        IN squery VARCHAR(255)
    )
BEGIN
SELECT * FROM `publication` WHERE `publication`.`title` like CONCAT('%',squery,'%');
END	;
...
END	;
#	PR`searchByName`cp1251_general_ci	;
...
WHERE o.`id` = ID;
END	;
#	PR`updatePublicationById`cp1251_general_ci	;
...
IF STAT = '1' THEN	
	...
	ap.`id_author` = (SELECT `acl_users`.`id` FROM `acl_users`
	WHERE `acl_users`.`username` = USERNAME);
ELSE
    DELETE  FROM `author_publication` 
	WHERE  `author_publication`.`id_publication` = ID AND
	`author_publication`.`id_author` = (SELECT `acl_users`.`id` FROM `acl_users`
	WHERE `acl_users`.`username` = USERNAME);
...
WHERE  p.`id` = ID;
END	;
#	PR`updateReportBasement`cp1251_general_ci	;
...
END	;
#	PR`updateReportName`cp1251_general_ci	;
...
s_h.`KEY` = 'report.head';
END	;
#	PR`updateReportType`cp1251_general_ci	;
